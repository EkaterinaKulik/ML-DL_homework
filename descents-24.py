...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        if self.loss_function == LossFunction.MSE:
          loss = (1 / (2 * x.shape[0])) * np.sum((x.dot(self.w) - y) ** 2)
        elif self.loss_function == LossFunction.LogCosh:
           loss = (1 / x.shape[0]) * np.sum(np.log(np.cosh((x.dot(self.w) - y) ** 2)))
        return loss
...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.loss_function == LossFunction.MSE:
          ...
          gradient = (1 / x.shape[0]) * np.dot(x.T, np.tanh(y_pred - y))
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.loss_function == LossFunction.MSE:
          ...
          gradient = (1 / self.batch_size) * np.dot(x_batch.T, np.tanh(y_pred_batch - y_batch))
